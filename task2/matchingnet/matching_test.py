...
if __name__ == '__main__':
	...
	novel_imgs, novel_label, val_imgs, val_label = io_data.read_test2(data_path, os.path.join(data_path, 'novel'), sample = sample)
	...
	pred_val = []
	for i in range(int(val_imgs.shape[0]/batch_size) + 1):
		target_x = val_imgs[batch_size*i: min(val_imgs.shape[0], batch_size*(i+1))]
	...
	pred_val = np.vstack(pred_val)
	idx = np.argmax(pred_val, axis = 1)
	novel_label = novel_label[np.arange(0, novel_label.shape[0], sample)]
	pred_val = [novel_label[i] for i in idx]
	acc = compute_acc(pred_val, val_label)
	print("Acc: ", acc)
